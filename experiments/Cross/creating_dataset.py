...
parser.add_argument('--tolerance', type=float, default=1.0, help='Task goal tolerance')
...
for num_run in tqdm(range(args.num_runs), desc="Progress: ", leave=False):
    for i, btype in enumerate(behavior_types):
        ...
        np.nan_to_num(current_poses = np.array([fill_poses(run.poses.copy(),args.num_steps)]),0)
